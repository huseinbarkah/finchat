    ...
    <div className="min-h-screen bg-navy-950 overflow-x-hidden">
      ...
      <nav
        className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
          scrolled ? "bg-navy-900/90 backdrop-blur-xl border-b border-navy-850/60 shadow-lg" : "bg-transparent"
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            <a href="#features" className="text-sm text-slate-400 hover:text-white transition-colors">Fitur</a>
            <a href="#simulator" className="text-sm text-slate-400 hover:text-white transition-colors">Demo</a>
            <a href="#pricing" className="text-sm text-slate-400 hover:text-white transition-colors">Harga</a>
            <Link
              href="/dashboard"
          ...
          {/* Mobile menu button */}
          <button className="md:hidden text-slate-300 hover:text-white" onClick={() => setMobileNav(!mobileNav)}>
            <i className={`fa-solid ${mobileNav ? "fa-xmark" : "fa-bars"} text-xl`}></i>
          </button>
        </div>
        ...
        {mobileNav && (
          <div className="md:hidden bg-navy-900/95 backdrop-blur-xl border-t border-navy-850 px-6 py-4 space-y-3 animate-slide-down">
            <a href="#features" className="block text-sm text-slate-300 py-2" onClick={() => setMobileNav(false)}>Fitur</a>
            <a href="#simulator" className="block text-sm text-slate-300 py-2" onClick={() => setMobileNav(false)}>Demo</a>
            <a href="#pricing" className="block text-sm text-slate-300 py-2" onClick={() => setMobileNav(false)}>Harga</a>
            <Link href="/dashboard" className="block w-full text-center py-2.5 rounded-xl bg-whatsapp text-navy-950 font-bold text-sm">
              Buka Dashboard →
            </Link>
          </div>
        )}
      ...
      <section className="relative pt-32 pb-20 md:pt-44 md:pb-32 px-6">
        ...
        <div className="max-w-5xl mx-auto text-center relative">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-whatsapp/10 border border-whatsapp/20 mb-8 animate-fade-in">
            ...
            <span className="text-xs font-semibold text-whatsapp">Powered by Google Gemini AI</span>
          </div>
          ...
          <p className="text-lg md:text-xl text-slate-400 max-w-3xl mx-auto mb-10 leading-relaxed animate-slide-up" style={{ animationDelay: "0.1s" }}>
            Kirim chat seperti <span className="text-white font-semibold">&quot;beli kopi goceng&quot;</span> — AI kami langsung parsing, kategorikan, dan catat ke pembukuan digital Anda secara otomatis.
          </p>
          ...
          {/* Stats */}
          <div className="flex flex-wrap justify-center gap-8 md:gap-16 mt-16 animate-slide-up" style={{ animationDelay: "0.3s" }}>
            {[
              ...
              <div key={s.label} className="text-center">
                <p className="font-[var(--font-title)] font-extrabold text-2xl md:text-3xl text-white">{s.val}</p>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="font-[var(--font-title)] font-extrabold text-3xl md:text-4xl text-white mb-4">
              Satu Platform, <span className="text-whatsapp">Semua Keuangan</span>
            </h2>
            <p className="text-slate-400 max-w-2xl mx-auto">
              Didesain untuk BEM, UMKM, dan keuangan pribadi yang ingin otomatisasi pembukuan tanpa ribet.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              >
                <div className={`w-12 h-12 rounded-xl bg-gradient-to-tr ${f.color} flex items-center justify-center text-white text-lg mb-4 shadow-lg group-hover:scale-110 transition-transform`}>
                  <i className={`fa-solid ${f.icon}`}></i>
                </div>
                <h3 className="font-[var(--font-title)] font-bold text-white text-base mb-2">{f.title}</h3>
        ...
        <div className="max-w-5xl mx-auto relative">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-400 max-w-xl mx-auto">
              Ketik pesan pengeluaran atau pemasukan kasual seperti chat WhatsApp biasa. AI kami parsing secara instan.
            </p>
          </div>
      ...
      <section id="pricing" className="py-20 md:py-28 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-400">Mulai gratis, upgrade ketika bisnis Anda bertumbuh.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              >
                ...
                <h3 className="font-[var(--font-title)] font-bold text-white text-lg mb-1">{plan.name}</h3>
                <div className="flex items-baseline gap-1 mb-1">
                  <span className="font-[var(--font-title)] font-extrabold text-3xl text-white">{plan.price}</span>
      ...
      <footer className="border-t border-navy-850 py-12 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-6">
            ...
            <a href="#" className="text-xs text-slate-500 hover:text-slate-300 transition-colors">Dokumentasi API</a>
          </div>

          <p className="text-xs text-slate-600">© 2026 FinChat.AI — All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
      ...
      <div className="bg-[#0b141a] rounded-2xl overflow-hidden border border-emerald-800/30 shadow-2xl">
        ...
        <div className="bg-[#075e54] px-4 py-3 flex items-center gap-3 text-white">
          ...
          <div>
            ...
            <p className="text-[10px] text-emerald-200">Demo Interaktif • Tanpa Login</p>
          </div>
        </div>
        ...
        {/* Input */}
        <form onSubmit={handleSend} className="bg-[#1f2c34] px-3 py-2.5 flex items-center gap-2">
          <input
            value={input}
